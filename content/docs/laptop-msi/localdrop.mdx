...
Set the MSI as the Linux device and the MacBook (and phone) as peers. Keep the
token identical on all devices, but never put it in the handbook or infra repo.
The peer addresses are the **WireGuard mesh addresses** (`10.8.0.12` MSI,
`10.8.0.11` MacBook, `10.8.0.21` iPhone) — mesh addresses are stable by design
and survive DHCP re-leases, so `peer.host` does not go stale (unlike the old
LAN-IP approach). See [LocalDrop — phone](/docs/iphone/localdrop).
