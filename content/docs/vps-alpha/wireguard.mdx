...
Delta is deployed as the infrastructure peer at `10.8.0.2/32`. Its direct
...
flowchart LR
    subgraph clients["Trusted clients"]
        cachyos["MSI laptop<br/>10.8.0.12/32<br/>Delta peer active"]
        macbook["MacBook<br/>10.8.0.11/32<br/>Runtime verified"]
        gamma["Gamma<br/>10.8.0.3/32<br/>Runtime and persistence verified"]
        beta["Beta laptop / Proxmox<br/>10.8.0.4/32<br/>Runtime verified"]
        delta["Delta Raspberry Pi<br/>10.8.0.2/32<br/>Direct backup node"]
        blackarch["BlackArch VM<br/>10.8.0.101/32<br/>SSH and runtime verified"]
        iphone["iPhone<br/>10.8.0.21/32<br/>Runtime restored 2026-07-24"]
    end
    ...
    gamma -->|"Encrypted tunnel<br/>VPS-only route"| wg
    beta -->|"Encrypted tunnel<br/>VPS-only route"| wg
    delta -->|"Encrypted tunnel<br/>VPS-only route"| wg
    blackarch -->|"Encrypted tunnel<br/>VPS-only route"| wg
    iphone -->|"Encrypted tunnel<br/>VPS-only route"| wg
    wg -->|"Private service traffic"| ollama
    macbook -. "No peer route and UFW routed deny" .-> cachyos
    iphone -. "No peer route and UFW routed deny" .-> macbook
```

| Machine | Address | Verification |
| --- | --- | --- |
| VPS | `10.8.0.1/24` | Runtime and reboot persistence verified |
| MSI laptop | `10.8.0.12/32` | Alpha and Delta peers active; direct handshake and ping verified 2026-08-13; reboot persistence pending |
| MacBook | `10.8.0.11/32` | Runtime and VPS persistence verified; Mac reboot activation pending |
| Gamma | `10.8.0.3/32` | Alpha path verified; Delta peer, direct ping, and Delta→Gamma SSH verified 2026-08-13; Gamma reboot persistence pending |
| Beta | `10.8.0.4/32` | Beta reboot persistence, handshake, bidirectional ping, Handbook access, Alpha-to-Proxmox access, and Mac SSH jump verified 2026-07-22; Alpha peer owner-applied and Alpha reboot proof pending |
| iPhone | `10.8.0.21/32` | Runtime identity rotation and private-service access verified 2026-07-24; obsolete peer cleanup and updated persistent-file proof pending |
| BlackArch VM | `10.8.0.101/32` | Direct VM peer; SSH through Alpha, active WireGuard, and key-only access verified 2026-08-12 |
| Delta Raspberry Pi | `10.8.0.2/32` | LAN SSH, active WireGuard, Alpha handshake, and direct peer definitions verified; reciprocal rollout pending |
...
DNS-only name `delta-vpn.zero-five.space`, UDP `51821` forwarded to
`192.168.1.106`, and direct `/32` peers for registered nodes.

The redundancy model is:

1. Clients connect to Alpha at `10.8.0.1` as the primary path.
2. Clients with Delta's reciprocal peer block can connect directly to
   `10.8.0.2` when Alpha is unavailable.
...
### Optional iPhone Alpha-relay fallback

The direct iPhone profile remains the primary path (`10.8.0.21/32` → Delta).
For networks that cannot reach Delta's residential public endpoint, an optional
second profile is prepared in the infra repository:

```text
wireguard/iphone-alpha-relay.conf.example
```

It uses `10.8.0.22/32` and connects to Alpha at `95.111.229.142:51820`. Alpha
then forwards `10.8.0.22` to Delta over the existing `10.8.0.1 ↔ 10.8.0.2`
mesh peer. Delta's Alpha peer must include `10.8.0.22/32` so replies return
through Alpha. This address separation preserves the direct `.21` route and
allows the user to enable the relay only when needed; WireGuard does not
perform automatic failover between duplicate destinations.

Activation requires one-time root changes on Alpha and Delta: add `.22/32` to
the existing phone peer on Alpha, add `.22/32` to Alpha's peer on Delta, and
allow only `wg0`→`wg0` forwarding on Alpha. The runtime changes should be
applied with `wg set` and persisted in the root-only configs; do not restart
`wg0` over a mesh SSH session.
...
After each client, verify `ping -c 3 10.8.0.2`, a recent `wg show` handshake,
...
> Boot activation therefore failed, and Ollama could not bind to `10.8.0.1`.
> The route was corrected to the modeled `10.8.0.21/32`, the complete file was
...
- Alpha exposes WireGuard UDP `51820`; Delta exposes UDP `51821` through a DNS-only Cloudflare record.
...
- Beta routes only `10.8.0.1/32`; its normal LAN and internet traffic do not
  cross the tunnel.
- The iPhone has only a `10.8.0.1/32` client route; the VPS's routed deny keeps
