...
flowchart TD
    ...
    hypr --> apps["Foot, Firefox, Code, Dolphin"]
...
- Foot as terminal, Firefox as browser, Code as editor, and Dolphin as file
...
- four-finger workspace swipes and three-finger gestures, with US/Spanish
  keyboard switching and flat pointer acceleration;
- media, screenshot, clipboard, emoji, session, lock, and workspace bindings;
- `Super+Shift+B/C/G` battery, calm, and gaming modes;
- `Super+Shift+0/1/2/D` automatic, single, dual, and toggled display layouts;
- startup display-layout, monitor-hotplug, and lid-state watchers;
...
The tracked personal overlay is loaded after Caelestia's generated modules.
Generated `.conf` files are not the active source for these settings. The
overlay starts the display helpers and calls the authored scripts under
`zero-five-infra/settings/msi/hypr/scripts/`. `hyprctl monitors` and
`cat /tmp/hypr-mode /tmp/hypr-display-layout` show the effective display state.

The display helper uses the laptop's `eDP-1` and external `DP-1` names, moves
workspaces before disabling the laptop panel, and skips lid-triggered layout
changes while the session is locked. If a dock exposes a different output
name, inspect `hyprctl monitors` before using a layout mode. The scripts do not
replace systemd/logind's own suspend policy.
...
```bash
hyprctl monitors
hyprctl binds | rg 'mode-|display-layout|safe-lock'
cat /tmp/hypr-mode /tmp/hypr-display-layout
hyprctl reload
