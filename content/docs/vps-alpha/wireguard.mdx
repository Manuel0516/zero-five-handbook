...
Delta is deployed as the infrastructure peer at `10.8.0.2/32` and listens on
UDP `51821` through `delta-vpn.zero-five.space`. Its Alpha handshake is
verified. MSI is the first client with a reciprocal Delta peer; the remaining
client rollout is incomplete.
The BlackArch VM on Beta is registered as a direct VM client at `10.8.0.101/32`.
