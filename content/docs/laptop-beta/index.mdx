...
flowchart LR
    ...
    alpha["Alpha VPS<br/>WireGuard hub<br/>10.40.0.1"]

    mac -->|"SSH alias delta<br/>WireGuard 10.40.0.2"| delta["Delta Raspberry Pi"]
    delta -->|"ProxyJump second hop<br/>10.40.0.4:22"| beta
    beta -->|"WireGuard client<br/>10.40.0.4/32"| alpha
...
| LAN | Static `192.168.1.166/24` on bridge `vmbr0`; gateway and DNS `192.168.1.1`; `nic0` MAC `d0:bf:9c:16:24:b8` |
| Wake-on-LAN | Ethernet magic packets enabled on `nic0`; PCI wake state verified `enabled`; suspend helper installed; functional suspend/wake test pending |
| WireGuard | Beta `wg0` at `10.40.0.4/32`; boot persistence, recent handshake, bidirectional ping, and private Handbook access verified after reboot |
| Mac SSH | `ssh beta` uses `ProxyJump delta`; Beta `root` authentication verified 2026-08-13 |
...
| Administration | Local OS accounts `root` and `manuel`; only Proxmox user `root@pam` |
...
`manuel` is the normal administrative account and `root` remains available for
recovery. Remote SSH accepts public keys and rejects password authentication.
Keep the LAN session or physical console available while changing firmware
virtualization, bridges, SSH, or firewall policy.
...
| [Media VM](/docs/laptop-beta/media-vm) | Overview of the guest plus dedicated VPN, qBittorrent, Tor Browser, security, recovery, and resource pages |
| [Wake-on-LAN](/docs/laptop-beta/wake-on-lan) | Magic-packet wake, Delta relay, and safe suspend helper |
