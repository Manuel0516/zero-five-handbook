...
The personal-ai router on Alpha is designed to merge models from two Ollama
backends:

```mermaid
flowchart LR
    browser["VPN client"] --> webui["Open WebUI<br/>ai.zero-five.space"]
    webui --> router["ollama-router<br/>Alpha 127.0.0.1:11435"]
    router --> server["Alpha Ollama<br/>10.8.0.1:11434"]
    router --> msi["MSI Ollama<br/>10.8.0.2:11434<br/>planned"]
```

The MSI backend is private and reachable only through the WireGuard route.
The current inventory found no local models, so the laptop branch currently
returns no model inventory until models are deliberately installed.
...
[Service]
Environment="OLLAMA_HOST=10.8.0.2:11434"
...
ollama list
ollama ps
curl --fail http://10.8.0.2:11434/api/tags
```
...
```bash
OLLAMA_HOST=10.8.0.2:11434 ollama list
```
...
3. Bring up and verify WireGuard first; Ollama cannot bind to `10.8.0.2`
...
| --- | --- | --- |
| Service fails to start | `wg0` or `10.8.0.2` absent | `ip address show wg0`, `journalctl -u ollama -b` |
| Local tags work but Alpha cannot see models | WireGuard route, firewall, or bind address | `ping 10.8.0.1`, `curl http://10.8.0.2:11434/api/tags` from Alpha |
