...
stable public hub; the MSI laptop (tracked by the legacy `cachyos-laptop`
template) and MacBook connect to it as clients.
...
flowchart LR
    subgraph clients["Trusted clients"]
        cachyos["MSI laptop<br/>10.8.0.2/24<br/>Live verification pending"]
...
| MSI laptop | `10.8.0.2/24` | Existing legacy CachyOS configuration documented; live re-verification pending |
...
| `wireguard/gamma.conf.example` | Gamma interface and VPS-only peer |
| `wireguard/beta.conf.example` | Beta interface and VPS-only peer |
| `wireguard/iphone.conf.example` | iPhone interface and VPS-only peer |
| `wireguard/blackarch.conf.example` | BlackArch VM interface and VPS-only peer |
