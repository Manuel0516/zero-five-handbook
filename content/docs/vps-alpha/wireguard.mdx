...
flowchart LR
    subgraph clients["Trusted clients"]
        msi["MSI laptop<br/>10.8.0.12/32<br/>AI backend (Ollama)<br/>peers: Alpha + Delta + Gamma"]
        macbook["MacBook<br/>10.8.0.11/32 direct<br/>10.8.0.31/32 VPS relay"]
        gamma["Gamma<br/>10.8.0.3/32<br/>AI host + Minecraft<br/>peers: Alpha + Delta + MSI"]
        beta["Beta / Proxmox<br/>10.8.0.4/32<br/>peers: Alpha + Delta"]
        delta["Delta Raspberry Pi<br/>10.8.0.2/32<br/>Infra node + Mullvad gateway"]
        blackarch["BlackArch VM<br/>10.8.0.101/32"]
        iphone["iPhone<br/>10.8.0.21/32 direct<br/>10.8.0.32/32 VPS relay"]
    end

    subgraph vps["VPS hub (Alpha)"]
        wg["WireGuard wg0<br/>10.8.0.1/24<br/>UDP 51820"]
        ollama["Ollama<br/>10.8.0.1:11434<br/>translategemma only"]
    end

    subgraph ai["AI mesh (2026-08-24)"]
        gwui["Open WebUI on gamma<br/>10.8.0.3:3000"]
        router["ollama-router on gamma<br/>127.0.0.1:11435"]
        gm["gamma Ollama<br/>10.8.0.3:11434<br/>qwen2.5:3b, nomic-embed"]
        msm["MSI Ollama<br/>10.8.0.12:11434<br/>deepseek-r1:7b, qwen2.5-coder:7b"]
    end
...
| --- | --- | --- | --- |
| Alpha VPS | `10.8.0.1/24` | WireGuard hub, Traefik edge, translation bot | all clients |
| Delta Pi | `10.8.0.2/32` | Infra node: AdGuard DNS, app gateway, backup controller, Mullvad gateway | Alpha + registered clients |
| Gamma | `10.8.0.3/32` | AI host (Open WebUI, ollama-router, SearXNG, Ollama qwen2.5:3b/nomic-embed-text), Minecraft | Alpha + Delta + **MSI** |
| Beta / Proxmox | `10.8.0.4/32` | Proxmox VE host, BlackArch VM parent | Alpha + Delta |
| MacBook | `10.8.0.11/32` direct; `10.8.0.31/32` VPS relay | Admin + dev box | Alpha + Delta |
| MSI | `10.8.0.12/32` | **AI backend** (Ollama deepseek-r1:7b, qwen2.5-coder:7b), personal desktop | Alpha + Delta + **Gamma** |
| iPhone | `10.8.0.21/32` direct; `10.8.0.32/32` VPS relay | Mobile access | Alpha + Delta |
| BlackArch VM | `10.8.0.101/32` | Security research VM (on Beta) | Alpha + Delta |
