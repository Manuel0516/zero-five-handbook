...
flowchart LR
    ...
    gamma["Gamma<br/>10.40.0.3<br/>AI host"]
    router["ollama-router on gamma<br/>127.0.0.1:11435"]
    webui["Open WebUI<br/>ai.zero-five.space"]
    alpha["Alpha VPS<br/>10.40.0.1<br/>WireGuard hub"]
    mac["MacBook<br/>administration"]
    localdrop["LocalDrop<br/>clipboard + files"]

    msi <-->|"mutual WG peer<br/>10.40.0.12 ⇄ 10.40.0.3<br/>LAN 51820"| gamma
    msi -->|"Ollama 10.40.0.12:11434<br/>deepseek-r1:7b, qwen2.5-coder:7b"| router
    gamma --> router
    router --> webui
    webui -. "Traefik on Alpha" .-> alpha
    msi -->|"peer 10.40.0.12"| alpha
    mac -->|"local network"| localdrop
    msi -->|"local network"| localdrop
```

The MSI is a personal Linux desktop **and a live AI backend**. It runs
Ollama (`10.40.0.12:11434`) with `deepseek-r1:7b` and `qwen2.5-coder:7b`, and
since 2026-08-24 it is wired to gamma through a **mutual WireGuard peer**
(default port 51820, LAN endpoints) so gamma's ollama-router merges the MSI
models into Open WebUI — the laptop branch is fully deployed and verified
(`MSI_LEG_WORKS` chat reply).
...
| WireGuard | MSI migration target at `10.40.0.12/32`; live cutover pending |
...
| [WireGuard](/docs/laptop-msi/wireguard) | MSI peer migration to `10.40.0.12`, narrow routing, verification, and rollback |
