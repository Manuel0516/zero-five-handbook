...
description: Configure the iPhone as 10.8.0.21 with a narrow route to only the VPS, then deploy and verify the peer safely.
---

The iPhone is deployed as `10.8.0.21/32`. Its WireGuard peer advertises only
...
| `zero-five-infra/wireguard/vps.conf.example` | Reserved iPhone peer and `10.8.0.21/32` route on the hub |
