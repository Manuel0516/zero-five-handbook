...
Gamma is recorded as desired state, while the iPhone peer route is persistent
on Alpha but still needs a handset handshake test.
...
flowchart LR
    subgraph clients["Trusted clients"]
        ...
        gamma["Gamma<br/>10.8.0.4/32<br/>Deployment pending"]
        iphone["iPhone<br/>10.8.0.10/32<br/>Persistent route; handshake pending"]
    end
    ...
    gamma -->|"Encrypted tunnel<br/>VPS-only route; planned"| wg
    iphone -->|"Encrypted tunnel<br/>VPS-only route; planned"| wg
...
| Gamma | `10.8.0.4/32` | Identity recorded; deployment and runtime verification pending |
| iPhone | `10.8.0.10/32` | Persistent peer route active on Alpha; handset handshake still needs verification |
...
contain public keys because WireGuard needs them, while the undeployed iPhone
uses `IPHONE_PUBLIC_KEY` until its app generates the identity. Private keys are
always explicit placeholders. Real files live on their machines with mode
`0600`, or inside the iOS app, and never enter Git.
