...
```mermaid
flowchart LR
    mac["MacBook<br/>192.168.1.157<br/>WireGuard 10.40.0.11"]
    alpha["Alpha VPS<br/>Public SSH :2020<br/>WireGuard 10.40.0.1"]
    gamma["Gamma<br/>LAN 192.168.1.107<br/>WireGuard 10.40.0.3"]

    mac -->|"SSH alias: delta"| delta["Delta Raspberry Pi<br/>WireGuard 10.40.0.2"]
    delta -->|"ProxyJump second hop<br/>10.40.0.3:22"| gamma
...
| Hostname | `gamma` |
| LAN | `enp2s0`, DHCP address `192.168.1.107/24` (DHCP-reserved; previously `.164`) |
| WireGuard | `wg0` at `10.40.0.3/32`; `wg-quick@wg0` enabled and active |
| VPN proof | Gamma pinged Alpha at `10.40.0.1`; Alpha pinged Gamma at `10.40.0.3` |
| SSH | `ssh gamma` uses `ProxyJump delta`; direct Delta→Gamma SSH verified 2026-08-13 |
...
| [Monero mining](/docs/laptop-gamma/monero-mining) | Disabled XMRig service, MSI pool/wallet configuration handoff, and later operation |
| [Minecraft](/docs/laptop-gamma/minecraft) | LynxHouse Fabric server, migrated from alpha 2026-08-23; -Xmx4G heap, DNS, reachability |
| [AI Stack](/docs/laptop-gamma/ai-stack) | personal-ai (Open WebUI + Ollama + router + SearXNG), migrated from alpha 2026-08-24; RAM budget with Minecraft |
| [Beatbump](/docs/laptop-gamma/beatbump) | Private YouTube Music frontend with VPN-only mobile playback and downloads |
| [Animated Figures](/docs/laptop-gamma/animated-figures) | Faceless YouTube AI video pipeline, migrated from alpha 2026-08-24 |
