...
The MSI is represented by the legacy `cachyos-laptop` template in
`zero-five-infra`. The intended peer address is `10.8.0.2/32`; Alpha remains
the hub at `10.8.0.1/24`.

```mermaid
flowchart LR
    msi["MSI laptop<br/>10.8.0.2/32<br/>candidate peer"] -->|"AllowedIPs: 10.8.0.1/32<br/>keepalive 25s"| wg["Alpha wg0<br/>10.8.0.1/24<br/>UDP 51820"]
...
| Source | Purpose |
| --- | --- |
| `zero-five-infra/wireguard/cachyos-laptop.conf.example` | Current legacy MSI peer template; private key remains a placeholder |
| `zero-five-infra/wireguard/vps.conf.example` | Alpha peer model and `10.8.0.2/32` ownership |
...
- The Ollama API should bind to `10.8.0.2`, never the public interface.
