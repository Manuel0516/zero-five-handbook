...
`zero-five-infra`. Its address is `10.40.0.12/32`. It has three WireGuard
peers: Alpha at `10.40.0.1`, Delta at `10.40.0.2`, and **Gamma at `10.40.0.3`
(mutual peer, added 2026-08-24)**.

```mermaid
flowchart LR
    msi["MSI laptop<br/>10.40.0.12/32"] -->|"Primary peer<br/>10.40.0.1/32"| alpha["Alpha VPS<br/>10.40.0.1/24<br/>UDP 51820"]
    msi -->|"Direct peer<br/>10.40.0.2/32"| delta["Delta Raspberry Pi<br/>10.40.0.2/32<br/>UDP 51820"]
    msi <-->|"Mutual peer<br/>10.40.0.3/32<br/>LAN 192.168.1.110:51820 ⇄ .107:51820"| gamma["Gamma<br/>10.40.0.3/32<br/>AI host"]
    gamma -->|"Ollama 10.40.0.12:11434"| msi
```
...
does not become a bridge to other clients. The gamma peer exists **only** to
carry Ollama traffic to the gamma router — its `AllowedIPs` is the single
`10.40.0.3/32`.

## Current state

The MSI-side reciprocal Delta peer is active. The direct MSI↔Delta handshake,
the `10.40.0.2/32` route, and bidirectional ping were verified on 2026-08-13.
...
| Source | Purpose |
| --- | --- |
| `zero-five-infra/wireguard/cachyos-laptop.conf.example` | Legacy-named MSI peer template at `10.40.0.12/32`; private key remains a placeholder |
| `zero-five-infra/wireguard/vps.conf.example` | Alpha peer model and `10.40.0.12/32` ownership |
...
- The Ollama API should bind to `10.40.0.12`, never the public interface.
...
## 2026-08-24: LAN endpoints + gamma mutual peer

Two changes on this date (see [gamma AI stack](/docs/laptop-gamma/ai-stack)):

1. **Delta peer endpoint → LAN IP.** The home router's hairpin NAT is broken
   for WireGuard, so `Endpoint = delta-vpn.zero-five.space:51820` (public
   DDNS) failed from inside the LAN — the direct Delta handshake died and DNS
   via AdGuard (`10.40.0.2`) broke. Fix: `Endpoint = 192.168.1.106:51820`.
   Roaming: swap back to the DDNS name away from home.
2. **Gamma mutual peer** (default port 51820, LAN endpoint `192.168.1.107`),
   so the personal-ai router on gamma can reach the MSI's Ollama
   (`deepseek-r1:7b`, `qwen2.5-coder:7b`). MSI UFW now allows `11434` from
   `10.40.0.3/32` in addition to `10.40.0.1/32` (alpha).
