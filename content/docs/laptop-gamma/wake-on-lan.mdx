...
## Wake Gamma from Delta or the MacBook

Delta can wake Gamma while the MacBook is away because Delta is an always-on
LAN relay. Install the tracked sender on Delta as `/usr/local/bin/delta-wake`,
then run:
...
The MacBook must otherwise be on the same `192.168.1.0/24` LAN. Install its
sender once:
...
The SSH clients should close. From Delta:
