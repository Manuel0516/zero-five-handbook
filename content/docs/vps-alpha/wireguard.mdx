...
DNS-only name `delta-vpn.zero-five.space`, UDP `443` forwarded to
...
The direct profiles remain primary:

- iPhone: `10.8.0.21/32` → Delta
- MacBook: `10.8.0.11/32` → Delta
...
They use `10.8.0.22/32` and `10.8.0.23/32`, respectively, and connect to Alpha
at `95.111.229.142:51820`. Their `AllowedIPs` include both Alpha (`10.8.0.1/32`)
and Delta (`10.8.0.2/32`), so VPS-hosted services remain reachable while Alpha
forwards the fallback addresses to Delta over the
existing `10.8.0.1 ↔ 10.8.0.2` mesh peer. Delta's Alpha peer must include both
...
the iPhone peer and `.23/32` to the MacBook peer on Alpha, add both addresses
...
- Alpha exposes WireGuard UDP `51820`; Delta exposes UDP `443` through a DNS-only Cloudflare record.
