...
flowchart LR
    ...
    router --> server["gamma Ollama<br/>10.40.0.3:11434"]
    router --> msi["MSI Ollama<br/>10.40.0.12:11434"]
```
...
[Service]
Environment="OLLAMA_HOST=10.40.0.12:11434"
Environment="OLLAMA_KEEP_ALIVE=1m"
Environment="OLLAMA_NUM_CTX=65536"
```

MSI's UFW permits TCP `11434` on `wg0` from gamma's `10.40.0.3/32` and
alpha's `10.40.0.1/32` sources:

```bash
sudo ufw allow in on wg0 from 10.40.0.1 to any port 11434 proto tcp
sudo ufw allow in on wg0 from 10.40.0.3 to any port 11434 proto tcp
sudo ufw reload
```
...
ollama list
ollama ps
curl --fail http://10.40.0.12:11434/api/tags
```
...
```bash
OLLAMA_HOST=10.40.0.12:11434 ollama list
```
...
3. Bring up and verify WireGuard first; Ollama cannot bind to `10.40.0.12`
...
| --- | --- | --- |
| Service fails to start | `wg0` or `10.40.0.12` absent | `ip address show wg0`, `journalctl -u ollama -b` |
| Local tags work but Alpha cannot see models | WireGuard route, UFW rule, or bind address | `ping 10.40.0.1`, `sudo ufw status`, `curl http://10.40.0.12:11434/api/tags` from Alpha |
